...
namespace SpecificOptimizers {
    static void
    headRedefine(BinaryTree<ExprNode> *&head, BinaryTree<ExprNode> *newHead){
        auto* prevHead = head;
        head = newHead;
        prevHead->setLeft(nullptr);
        prevHead->setRight(nullptr);
        BinaryTree<ExprNode>::Delete(prevHead);
    }
